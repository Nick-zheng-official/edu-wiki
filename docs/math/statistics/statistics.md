...
其中 $\Omega$ 为样本空间, 其中的样本点发生概率相等. 需要注意的是, 以抽球问题为例, 若一种颜色的球有多个, 则需要对每一个球进行标号列入样本空间, 才能保证每一样本点概率相等. 
...
伯努利试验: 只包含两个可能结果的随机试验. $n$ 次独立重复伯努利试验计算概率十分简单: $P(X
 = k) = C_n^k p^k (1 - p)^{(n - k)}$ , 此时(试验间必须独立, 概率稳定) $X$ 服从二项分布, 表示为 $X \sim B(n, p)$ . 有性质 $E(X) = np, D(X) = np(1 - p)$ . 有时最后一次是特殊的(如几局几胜, 谁胜利, 最后一局必然谁赢), 前几次服从二项分布, 可以分别考虑.
