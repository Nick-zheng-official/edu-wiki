...
[【导数热门：极值点偏移问题，一个视频讲懂！】](https://www.bilibili.com/video/BV1K4421f7zT/?share_source=copy_web)  
...
其实本质就是极值点两侧各有单调性, 只要用题目条件把$x_1$和$x_2$首先放到不等号 **两边** , 然后发现单调性一致(必须得放在 **同一个单调区间** 内)就可以构造函数然后$"$穿衣服$"$了, 然后使用题目给的$f(x_1)$和$f(x_2)$的关系消元即可得到一个只关于一个变量的不等式, 然后就变成证明不含参不等式的题目了.
