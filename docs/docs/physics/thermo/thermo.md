...
温度在宏观上表示物体的冷热程度, 在微观上表示分子热运动的剧烈程度. 常见的温标有摄氏温度 $t$ , 单位 $\celsius$ 与热力学温度 $T$ , 单位 $K$ (开尔文) . 符合 $T = t + 273.15$ , 一般近似认为 $T = t + 273$ . 故常见 $27\celsius$ , 因为 $27 + 273 = 300$ . 温度的最低值为绝对零度( $0 K$ ), 即热运动消失, 故绝对零度无法达到, 且热力学温度没有负数.
...
内能判断题目可以分为固液(不考虑 $W$ 变化)与气体(考虑 $W$ 变化)两类, 其中固液题目会问晶体熔化时其内能变化(吸收热量内能增大, 由于温度不变则分子动能不变, 空间点阵结构破坏导致分子势能增大); 液体题目会问等质量 $0\celsius$ 的水与冰谁的内能大, 同理分析, 冰吸热变为水, 故水的内能较大.
